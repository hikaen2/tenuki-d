...
private const short[1548][81] PV;
...
short staticValue(const ref Position p) {
    ...
    for (type_t t = Type.PAWN; t <= Type.GOLD; t++) {
        value += PV[bk][OFFSET_HAND[Side.BLACK][t] + p.piecesInHand[Side.BLACK][t]];
        value += PV[bk][OFFSET_HAND[Side.WHITE][t] + p.piecesInHand[Side.WHITE][t]];
        value -= PV[wk][OFFSET_HAND[Side.BLACK][t] + p.piecesInHand[Side.WHITE][t]];
        value -= PV[wk][OFFSET_HAND[Side.WHITE][t] + p.piecesInHand[Side.BLACK][t]];
    }
    for (int i = 11; i <= 99; i++) {
        if ((p.squares[i].isBlack() || p.squares[i].isWhite()) && p.squares[i].type() != Type.KING) {
            value += PV[bk][OFFSET_BOARD[p.squares[i]             ] + ADDRESS_OF[i      ]];
            value -= PV[wk][OFFSET_BOARD[p.squares[i] ^ 0b00010000] + ADDRESS_OF[110 - i]];
        }
    }

    value /= 32;

    return cast(short)(result + value);
}
